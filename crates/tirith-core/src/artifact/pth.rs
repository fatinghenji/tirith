...
fn is_canonical_namespace_template(norm: &str) -> bool {
    ...
    // Current setuptools removed the Python-3.5 `has_mfs` compatibility branch
    // and joins the exact statement tuple with bare semicolons. Dotted namespaces
    // append one exact parent/child binding. Regenerate the COMPLETE upstream line
    // from the already-validated namespace; any inserted/reordered statement or an
    // inconsistent parent/child literal therefore still invalidates the exemption.
    let current_prefix = format!(
        "import sys, types, os;p = os.path.join(sys._getframe(1).f_locals['sitedir'], *{tuple});importlib = __import__('importlib.util');__import__('importlib.machinery');"
    );
    let mut current = format!(
        "{current_prefix}m = sys.modules.setdefault('{namespace}', importlib.util.module_from_spec(importlib.machinery.PathFinder.find_spec('{namespace}', [os.path.dirname(p)])));m = m or sys.modules.setdefault('{namespace}', types.ModuleType('{namespace}'));mp = (m or []) and m.__dict__.setdefault('__path__',[]);(p not in mp) and mp.append(p)"
    );
    if let Some((parent, child)) = namespace.rsplit_once('.') {
        current.push_str(&format!(
            ";m and setattr(sys.modules['{parent}'], '{child}', m)"
        ));
    }
    norm == current
}
...
mod tests {
    ...
    #[test]
    fn current_setuptools_namespace_templates_are_benign_only_when_exact() {
        // Generated by setuptools' current `Installer._gen_nspkg_line`: the modern
        // template removed the legacy `has_mfs` branch, and a dotted namespace adds
        // one exact parent/child binding statement.
        let top_level = "import sys, types, os;p = os.path.join(sys._getframe(1).f_locals['sitedir'], *('acme',));importlib = __import__('importlib.util');__import__('importlib.machinery');m = sys.modules.setdefault('acme', importlib.util.module_from_spec(importlib.machinery.PathFinder.find_spec('acme', [os.path.dirname(p)])));m = m or sys.modules.setdefault('acme', types.ModuleType('acme'));mp = (m or []) and m.__dict__.setdefault('__path__',[]);(p not in mp) and mp.append(p)";
        let nested = "import sys, types, os;p = os.path.join(sys._getframe(1).f_locals['sitedir'], *('acme', 'plugins'));importlib = __import__('importlib.util');__import__('importlib.machinery');m = sys.modules.setdefault('acme.plugins', importlib.util.module_from_spec(importlib.machinery.PathFinder.find_spec('acme.plugins', [os.path.dirname(p)])));m = m or sys.modules.setdefault('acme.plugins', types.ModuleType('acme.plugins'));mp = (m or []) and m.__dict__.setdefault('__path__',[]);(p not in mp) and mp.append(p);m and setattr(sys.modules['acme'], 'plugins', m)";

        assert!(is_benign_template(top_level));
        assert!(is_benign_template(nested));

        let appended = format!("{nested};open('/tmp/owned', 'w').write('x')");
        assert!(!is_benign_template(&appended));
        let wrong_parent = nested.replace("sys.modules['acme']", "sys.modules['evil']");
        assert!(!is_benign_template(&wrong_parent));
        let wrong_child = nested.replace("'plugins', m", "'payload', m");
        assert!(!is_benign_template(&wrong_child));
    }
